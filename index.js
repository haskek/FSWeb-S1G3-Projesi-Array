...
function indekstekiCesitiGetir(tatlar,istenilentat){
  tatlar1=tatlar[istenilentat];
  return tatlar1;
}
...
function ismeGoreCesitCikar(tatlar,lezzetadı){
  tatlar1=tatlar.indexOf(lezzetadı,0);
  tatlar2=tatlar.splice(tatlar1,1);
  return tatlar
...
function ismeGoreFiltrele(tatlar,filtrecik){
  let a=[];
  for(let i=0; i< tatlar.length ; i++ ){
    if(tatlar[i].includes(filtrecik)) {
      a.push(tatlar[i]);
    }
  }
  return a
